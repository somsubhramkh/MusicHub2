...
cartApp.controller("addToCartCtrl", function ($scope, $http){
    ...
    $scope.removeItemFromCart = function (itemId) {
    	alert('remove called')
        $http.put('http://localhost:8080/musichub/usercart/cart/removeItem/'+itemId).success(function (data) {
    ...
    $scope.addItemToCart = function (itemId) {
        $http.put('http://localhost:8080/musichub/usercart/cart/addItem/'+itemId).success(function () {
    ...
    $scope.clearCartItems = function () {
    	$http['delete']('http://localhost:8080/musichub/usercart/cart/clearCartItems/'+$scope.cartId).success(function (data) {$scope.refreshCartItems()});
    };
});